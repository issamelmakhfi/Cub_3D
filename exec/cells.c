...
{
	if (!(ray->h_skip)
		&& ray->xcell_h >= ft_strlen(position->info->map_arr[ray->ycell_h]))
		ray->h_skip = 1;
	if (!(ray->v_skip)
		&& ray->xcell_v >= ft_strlen(position->info->map_arr[ray->ycell_v]))
		ray->v_skip = 1;
}
...
{
	if (ray->xcell_h >= position->info->map_w
		|| ray->ycell_h >= position->info->map_h)
		ray->h_skip = 1;
	if (ray->xcell_v >= position->info->map_w
		|| ray->ycell_v >= position->info->map_h)
		ray->v_skip = 1;
	if (ray->quadrant == 4)
	{
		if (ray->xcell_h > position->x_cell || ray->ycell_h > position->y_cell
