...
/*   Updated: 2023/03/06 15:57:21 by ielmakhf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	s_move(t_mlx *mlx, int x_save, int y_save)
{
	if (mlx->pos->_s)
	{
		...
		mlx->table->sin_table[(int)(mlx->pos->pov / ANG_IN_D)] * 3 \
		, y_save + mlx->table->cos_table[(int)(mlx->pos->pov / ANG_IN_D)] * 3))
			return (1);
		mlx->pos->virtual_px -= \
		mlx->table->sin_table[(int)(mlx->pos->pov / ANG_IN_D)] * 3;
		mlx->pos->virtual_py += \
		mlx->table->cos_table[(int)(mlx->pos->pov / ANG_IN_D)] * 3;
...
int	a_move(t_mlx *mlx, int x_save, int y_save)
{
	if (mlx->pos->_a)
	{
		if (map_collisions2(mlx, x_save - \
		(mlx->table->cos_table[(int)(mlx->pos->pov / ANG_IN_D)] * 3) \
		, y_save - mlx->table->sin_table[(int)(mlx->pos->pov / ANG_IN_D)] * 3))
			return (1);
		mlx->pos->virtual_px -= \
		mlx->table->cos_table[(int)(mlx->pos->pov / ANG_IN_D)] * 3;
		mlx->pos->virtual_py -= \
