...
/*   Updated: 2023/03/02 18:42:16 by ielmakhf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	project_wall(t_mlx *mlx, int i)
{
	...
	j = 0;
	if (((WIN_H - mlx->pos->wall_height) / 2) <= 0)
		j = -1 * (((WIN_H - mlx->pos->wall_height) / 2));
	while (j < mlx->pos->wall_height)
	{
		get_texture_color(mlx, j, i);
		mlx->pos->top_pixel = ((WIN_H - mlx->pos->wall_height) / 2) + j;
